...
<div id="main-wrapper">
	<a class="skip-link screen-reader-text" href="#content"><?php esc_html_e( 'Skip to content', 'tuomas' ); ?></a>
						...
						<div class="row site-branding-container">
							<div class="site-branding col-md-3 col-md-offset-0 col-sm-4 col-sm-offset-0 col-xs-9 col-xs-offset-3">
								...
								<a id="home-link" href="<?php echo home_url(); ?>">
									<img src="<?php echo home_url() . "/wp-content/themes/tuomas/assets/images/tuomas_kallinen.svg" ?>" width="250px">
								</a>
							...
							<div class="col-md-12">
								...
								$page_heading = $page_subheading = ''; 
									if ( is_search() ) :
										...
										$page_subheading = sprintf(esc_html('Search results for "%s"', 'tuomas'), $page_heading);
									elseif ( is_404() ) :
										$page_heading = '404 – Page not found';
										$page_subheading = esc_html("Nothing was found at this location.", 'tuomas'); 
									elseif ( is_category() ) :
										$category = get_the_category($post->ID);
										$page_heading = $category[0]->name;
										$page_subheading = sprintf(esc_html('All posts in category "%s"', 'tuomas'), $page_heading);
									elseif ( is_tag() ) :
										$page_heading = single_tag_title('', false);
										$page_subheading = sprintf(esc_html('All posts tagged "%s"', 'tuomas'), $page_heading);
									elseif ( is_author() ) :
										$author = get_userdata( get_query_var('author') );
										$page_heading = $author->nickname;
										$page_subheading = sprintf(esc_html('All posts written by %s', 'tuomas'), $page_heading);
									elseif (is_day()) :
										$page_heading = get_the_date();
										$page_subheading = esc_html('Daily archive', 'tuomas'); 
									elseif (is_month()) :
										$page_heading = get_the_date('F Y');
										$page_subheading = esc_html('Monthly archive', 'tuomas'); 
									elseif (is_year()) :
										$page_heading = get_the_date('Y');
										$page_subheading = esc_html('Yearly archive', 'tuomas'); 
									elseif ( is_home() ) :
										$page_heading = esc_html('Blog', 'tuomas');
									elseif ( is_single() && ! is_attachment() ):
										$category = get_the_category($post->ID);
										$page_heading = get_the_title();
										$page_subheading = $category[0]->name . ' | ' . tuomas_posted_on();	
									else :
										$page_heading = get_the_title();
									endif; ?>
